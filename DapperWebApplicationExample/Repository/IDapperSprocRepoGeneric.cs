...
{
    public interface IDapperSprocRepoGeneric
    {
        string ConnectionString { get; set; }

        void Execute(string name);
        void Execute(string name, object param);
        List<T> ListByProcedureName<T>(string name);
        ...
        T SingleByProcedureNameAndId<T>(string name, int id);
        T SingleByProcedureName<T>(string name, object param);
    }
}
